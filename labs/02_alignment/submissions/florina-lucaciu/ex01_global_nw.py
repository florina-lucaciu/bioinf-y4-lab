...
def init_score_matrix_global(m: int, n: int, gap: int):
    ...
    # creare matrice (m+1)x(n+1) plină cu 0
    score = [[0 for _ in range(n + 1)] for _ in range(m + 1)]

    # prima coloană: penalizări pentru gap-uri pe verticală
    for i in range(1, m + 1):
        score[i][0] = i * gap

    # prima linie: penalizări pentru gap-uri pe orizontală
...
def score_cell_global(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    ...
    # sus: gap în secvența 2
    up = score[i - 1][j] + gap

    # stânga: gap în secvența 1
    left = score[i][j - 1] + gap

    # alegem cel mai mare scor
